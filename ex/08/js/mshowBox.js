var mshow = (function($) {
    var mshowboxFun = {
        myshowBox: function(titles, urls, objWidth, objHeight, types, classname) {
            ...
            mshowboxFun.showPositionFix($("myshowbox" + _class));
        ...
        myshowBox2: function(obj) {
            ...
            $(".show-myshowbox2").live('click', function() {
                ...
                for (var i = 0; i < objName.length; i++) {
                    if ($(this).hasClass("show-" + objName[i])) {
                        mshowboxFun.showPositionFix($(".show-div-" + objName[i]));
            ...
            /*共用关闭*/
            $(".myshowbox .btn-close").live('click', function() {
        ...
        showPositionFix : function(obj) {   //弹出框位置Fix
            var obj = obj;
            var _wh = kboxing.findDimensions().split("-");
            /*box fixed*/
            var boxnFixed = function() {
                if (jQuery.browser.version == '6.0') {
                    obj.css({ "top": document.documentElement.scrollTop + (_wh[1] - obj.height()) / 2 + "px", "left": (_wh[0] / 2 + document.documentElement.scrollLeft) });
                }
            }
